...
```{r}
hist(data$course_rating, col = "#DCB7F9", border = "white", main="Distribution of courses' ratings", breaks = seq(1,5,0.1))
box(bty="l")
grid(nx=NA,ny=NULL,lty=1,lwd=1,col="lightgrey")
```
```{r}
hist(data$course_level, col = "#B7D079", border = "white", main="Distribution of courses' levels", breaks = seq(-1,3))
box(bty="l")
```
...
ggplot(data, aes(x=price,fill=factor(high)))+geom_density(alpha=0.5)+
 xlab(label = "Price")+
 ggtitle("Distribution of price taking course rating into account")
```
```{r}
...
ggplot(data_reviews, aes(x=coures_reviews,fill=factor(high)))+geom_density(alpha=0.5)+
 ...
 ggtitle("Distribution of number of reviews taking course rating into account")
```

```{r}
ggplot(data, aes(x=price,fill=factor(course_level)))+geom_density(alpha=0.5)+
 xlab(label = "Price")+
 ggtitle("Distribution of price taking course level into account")
```
